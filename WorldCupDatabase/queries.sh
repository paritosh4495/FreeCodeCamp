...
echo "$($PSQL "SELECT t.name FROM games g INNER JOIN teams t ON g.winner_id = t.team_id WHERE g.round = 'Final' AND g.year = 2018")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT DISTINCT t.name FROM games g INNER JOIN teams t ON g.winner_id = t.team_id OR g.opponent_id = t.team_id WHERE g.round = 'Eighth-Final' AND g.year = 2014 ORDER BY t.name")"
